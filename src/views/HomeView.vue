...
async function PredictClick() {
    ...
    for (let i = 0; i < max_users.value; i++) {
        ...
        let predictions_a, predictions_b;

        if (tasks_a.choices[0].message.content !== null) {
            predictions_a = JSON.parse(tasks_a.choices[0].message.content);
        } else {
            console.error("tasks_a content is null");
        }

        if (tasks_b.choices[0].message.content !== null) {
            predictions_b = JSON.parse(tasks_b.choices[0].message.content);
        } else {
            console.error("tasks_b content is null");
        }
        ...
        like_confidence.value = isNaN(like_confidence_results) ? 0 : like_confidence_results;
        like_winner.value = like_winner_results

        comment_confidence.value = isNaN(comment_confidence_results) ? 0 : comment_confidence_results;
        comment_winner.value = comment_winner_results

        share_confidence.value = isNaN(share_confidence_results) ? 0 : share_confidence_results;
        share_winner.value = share_winner_results

        quote_confidence.value = isNaN(quote_confidence_results) ? 0 : quote_confidence_results;
